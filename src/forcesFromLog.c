...
int main ( int argc, char * argv[] ) {
  ...
  if (nz==nth) {
    ...
    vecdiff(d,&th[0],&z[0]);
    memcpy(rsum[0].r,d->r,SDIM*sizeof(double));
    memcpy(ravg[0].r,d->r,SDIM*sizeof(double));
    for (i=1;i<nz;i++) {
      thp=&th[i]; zp=&z[i];
      vecdiff(d,thp,zp);
    ...
    for (i=0;i<nz;i++) {
      fprintf(fp,"%i ",i);
      for (j=0;j<SDIM;j++) fprintf(fp,"%.8lf ",ravg[i].r[j]);
      fprintf(fp,"\n");
    }
    fclose(fp);
    ...
    i--;
    thp=&th[i];
    for (j=0;j<SDIM;j++) fprintf(stdout,"%.8lf ",thp->r[j]);
    for (j=0;j<SDIM;j++) fprintf(stdout,"%.8lf ",ravg[i].r[j]);
    fprintf(stdout,"\n");
  }
